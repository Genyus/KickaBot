...
function loadFeed (options, callback) {
  ...
  request(url, function (err, response, body) {
    // Check for error
    if (err) {
      logger.warn(String.format('Error occurred while requesting {0}: ', url), err);
    ...
    if (response.statusCode !== 200) {
      logger.warn('Invalid response status code returned: ', response.statusCode);
    ...
    try {
      ...
      logger.warn(String.format('Error occurred while parsing JSON: {0}\r\nJSON:\r\n{1}', url), exception.message, body);
    ...
    if (feed.contents.ERROR === 'OK') {
      ...
      logger.warn(String.format('Unexpected response:\r\n{0}', body));
