...
var self = module.exports = {
    ...
    _.each(matches, function(standing, i) {
        rows.push([String.format('{0} vs {1}',
            ui.getShortName(standing.match_localteam_name),
            ui.getShortName(standing.match_visitorteam_name))]);
    });
     ...
     * @return {[type]}            [description]
     */
    renderMatchStats: function renderMatchStats(error, feed, teamName, callback) {
        var standings = feed.teams.filter(function(el) {
   ...
   renderResults: function renderResults(error, feed, callback) {
    ...
    renderTable: function renderTable(error, feed, callback) {
        //Check for error
        if (error) {
            return utils.log('Error in renderTable function', error);
        }
    ...
    writeRow: function writeRow(columns) {
        ...
        //       utils.log(String.format('Key: {0}, value: {1}', key, this[key]));
        //       var width = this[key];
